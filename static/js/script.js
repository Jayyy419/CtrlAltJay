...
function initAdminAuth() {
  const openButton = document.getElementById("open-admin-auth");
  ...
  if (!openButton || !modal || !form || !passcodeInput || !feedback) {
    return;
  }
  ...
  openButton.addEventListener("click", async () => {
    try {
      const statusResponse = await fetch("/api/admin/auth-status", { method: "GET" });
      if (statusResponse.ok) {
        const statusPayload = await statusResponse.json();
        if (statusPayload.is_admin) {
          setAdminMode(true);
          return;
        }
      }
    } catch (_) {
      // If status call fails, continue with popup flow.
    }

    openModal();
  });

  modal.querySelectorAll("[data-close-admin-auth]").forEach((element) => {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (!passcode) {
      feedback.textContent = "Passcode is required.";
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        feedback.textContent = payload.error || "Authentication failed.";
        return;
      }

      setAdminMode(true);
      feedback.textContent = "Verified. Editing enabled.";
      closeModal();
    } catch (_) {
      feedback.textContent = "Could not verify right now. Please try again.";
    }
  });
